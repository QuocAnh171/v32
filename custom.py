...
class LeCartCustom(LeBasecart):
	def display_config_source(self):
		...
		config_queries = {
			'languages': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_languages WHERE languages_id",
			},
			'currencies': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_currencies WHERE currencies_id" ,
			},
		}
		...
		for language_row in config_data['languages']:
			language_data[language_row['languages_id']] = to_str(language_row['name']) + "(" + to_str(language_row['code']) + ")"
		for currency_row in config_data['currencies']:
			currency_data[currency_row['currencies_id']] = currency_row['title']
	...
	def display_config_target(self):
		...
		self._notice['target']['support']['skip_demo'] = False
		self._notice['target']['support']['customer_group_map'] = False
	...
	def get_categories_ext_export(self, categories):
		categories_ids = duplicate_field_value_from_list(categories['data'], 'categories_id')
		categories_ext_queries = {
			'categories_description': {
				'type': 'select',
				'query': "SELECT * FROM  categories_description WHERE categories_id IN " + self.list_to_in_condition(categories_ids)
			},
			'languages' : {
				'type' : 'select',
				'query' : "SELECT * FROM  languages lg INNER JOIN categories_description cd ON lg.languages_id = cd.language_id WHERE cd.categories_id IN" + self.list_to_in_condition(categories_ids)
			}
		}
	...
	def convert_category_export(self, category, categories_ext):
		...
		# 	lang = get_row_from_list_by_field(categories_ext['data']['languages'],'languages_id',language_id)
		# 	category_language_data['name'] = lang['name']
		# 	category_data['languages']['language_id'] = category_language_data
		# category_data['category'] = category
		category_data['languages']['language_id'] = self._notice['src']['languages'].items()
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'product_attribute': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_product_attribute WHERE products_id IN  " + self.list_to_in_condition(product_ids),
			},
		...
		if (not products_ext) or products_ext['result'] != 'success':
			return response_error("ERROR Products_Ext")
		attribute_ids = duplicate_field_value_from_list(products_ext['data']['product_attribute'],'attribute_id')
		product_ext_rel_queries = {
			'attribute': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_attribute WHERE attribute_id IN " + self.list_to_in_condition(attribute_ids),
			},
		}
		...
		if (not products_ext_rel) or products_ext_rel['result'] != 'success':
			return response_error("ERROR Product_Ext_Rel")
		products_ext = self.sync_connector_object(products_ext,products_ext_rel)
		return products_ext
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['price'] = product['products_price']
		product_description = get_row_from_list_by_field(products_ext_data['products_description'], 'products_id', product['products_id'])
		product_data['name'] = product_description['products_name']
		product_data['description'] = product_description['products_description']
		product_data['short_description'] = html.unescape(product_description['nutrition_facts_htm'])
		...
		product_data['languages'] = self._notice['src']['languages']
		if product['products_image_large']!='':
			images = dict()
			images['url'] = url_product_image
			images['path'] = product['products_image_large']
		...
		products_attribute = get_list_from_list_by_field(products_ext_data['product_attribute'], 'products_id', product['products_id'])
		for product_attribute in products_attribute:
			product_attribute_data = self.construct_product_attribute()
			attributes = get_row_from_list_by_field(products_ext_data['attribute'],'attribute_id',product_attribute['attribute_id'])
			product_attribute_data['option_id'] = attributes['attribute_id']
			product_attribute_data['option_type'] = attributes['attribute_type']
			product_attribute_data['option_code'] = attributes['attribute_code']
			product_attribute_data['option_name'] = attributes['attribute_name']
			product_attribute_data['option_set'] = product_attribute['value']
	...
	def check_product_import(self, convert, product, products_ext):
		id_imported = self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'], convert['code'])
		return id_imported
	...
	def get_customers_ext_export(self, customers):
		...
		if (not customers_ext) or customers_ext['result'] != 'success':
			return response_error("ERROR Customer_Ext")
		...
		customer_ext_rel_queries = {
			...
			'zones': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_zones WHERE zones_id IN " + self.list_to_in_condition(state_ids),
			},
		}
		customers_ext_rel = self.select_multiple_data_connector(customer_ext_rel_queries,'customers')
		if (not customers_ext_rel) or customers_ext_rel['result'] != 'success':
			return response_error("ERROR Customer_Ext_Rel")
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		if address_books:
			for address_book in address_books:
				...
				address_data['address_1'] = address_books['entry_street_address']
				...
				country = get_row_from_list_by_field(customers_ext['data']['countries'], 'countries_id',address_books['entry_country_id'])
				if country:
				...
				state_id = address_books['entry_zone_id']
				if state_id:
					state = get_row_from_list_by_field(customers_ext['data']['zones'], 'zone_code', state_id)
					if state:
						address_data['state']['id'] = state['zone_id']
						address_data['state']['state_code'] = address_books['zone_code']
						address_data['state']['name'] = state['zone_name']
					else:
						address_data['state']['id'] = state_id
						address_data['state']['name'] = address_books['entry_state']
				else:
					address_data['state']['name'] = 'AL'
				if address_books['address_book_id'] == customer['customers_default_address_id']:
	...
	def check_customer_import(self, convert, customer, customers_ext):
		id_imported = self.get_map_field_by_src(self.TYPE_CUSTOMER, convert['id'], convert['code'])
		return id_imported
	...
	def check_order_import(self, convert, order, orders_ext):
		id_imported = self.get_map_field_by_src(self.TYPE_ORDER, convert['id'], convert['code'])
		return id_imported
