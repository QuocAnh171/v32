...
class LeCartWoocommerce(LeCartWordpress):
	...
	def display_config_source(self):
		...
		queries_config = {
			'orders_status': {
				...
				'query': "SELECT * FROM _DBPRF_posts WHERE post_type = 'shop_order'",
			},
			'permalink_structure': {
				'type': 'select',
	...
	def get_query_display_import_source(self, update = True):
		compare_condition = ' > '
		if update:
			compare_condition = ' <= '
	...
	def clear_target_manufacturers_demo(self):
		...
		if manufacturers['data']:
			...
			if all_queries:
				self.import_multiple_data_connector(all_queries, 'clear_demo')
	...
	def clear_target_products_demo(self):
		...
		all_meta_query = {
			'type': 'query',
			'query': "DELETE FROM `_DBPRF_postmeta`"
	...
	def clear_target_customers_demo(self):
		...
		del_user_meta_query = "DELETE FROM _DBPRF_usermeta WHERE user_id > 1  IN " + customer_id_con
		clear_table_1 = self.get_connector_data(self.get_connector_url('query'), {
			'query': json.dumps({
		...
		if self._notice['target']['support'].get('plugin_woo_admin') or self.convert_version(self._notice['target']['config']['version'], 2) > 399:
			del_customer_lookup_query = "DELETE FROM _DBPRF_wc_customer_lookup WHERE user_id > 1 IN " + customer_id_con
			clear_table_2 = self.get_connector_data(self.get_connector_url('query'), {
				'query': json.dumps({
	...
	def clear_target_orders_demo(self):
		...
		all_meta_query = {
			'type': 'select',
			'query': "DELETE FROM `_DBPRF_postmeta` WHERE post_id IN " + self.list_to_in_condition(order_ids)
		}
		clear_table = self.get_connector_data(self.get_connector_url('query'),
		                                      {'query': json.dumps(all_meta_query)})
	...
	def clear_target_reviews_demo(self):
		...
		for table in tables:
			...
			if table == 'wp_commentmeta':
				where = " WHERE admincomment_id IN " + self.list_to_in_condition(review_ids)
	...
	def clear_target_manufacturers(self):
		...
		# 	query_wpseo = {
		# 		'type': 'select',
		# 		'query': "SELECT * FROM `_DBPRF_wp_options` WHERE `option_name` = 'wpseo_taxonomy_meta'"
		# 	}
	...
	def clear_target_products(self):
		...
		while self._check_product_exists():
			...
			all_collections_query = {
				'type': 'query',
				'query': "DELETE FROM `_DBPRF_posts` WHERE post_type IN('product', 'product_variation','attachment')"
			}
			clear_table = self.get_connector_data(self.get_connector_url('query'),
			                                      {'query': json.dumps(all_collections_query)})
			if (not clear_table) or (clear_table['result'] != 'success') or (not clear_table['data']):
			...
			if (not clear_table_1) or (clear_table_1['result'] != 'success'):
				self.log("Clear data failed. Error: Could not empty products", 'clear')
				continue
		# clear attributes
		del_transient_attr_query = {
		...
		while self._check_attributes_exists():
			...
			if (attributes['data']):
				...
				attribute_names = duplicate_field_value_from_list(attributes['data'], 'attribute_label')
				...
				if (terms['data']):
					...
					self.get_connector_data(self.get_connector_url('query'),
					                        {'query': json.dumps(del_transient_attr_query)})
					del_relationship_attr_query = {
						'type': 'query',
						'query': "DELETE FROM `_DBPRF_term_relationships` WHERE term_taxonomy_id IN " + self.list_to_in_condition(
							taxonomy_ids)
					}
					self.get_connector_data(self.get_connector_url('query'),
											{'query': json.dumps(del_relationship_attr_query)})
					del_transient_attr_query = {
						'type': 'query',
						'query': "DELETE FROM `_DBPRF_terms` WHERE term_id IN " + self.list_to_in_condition(
							term_ids)
	...
	def clear_target_customers(self):
		...
		del_user_query = "DELETE _DBPRF_users FROM _DBPRF_users " \
		                 ...
		                 "WHERE _DBPRF_usermeta.meta_key IN ('capabilities', 'wp_admincapabilities') " \
		...
		del_customer_lookup_query = "DELETE _DBPRF_wc_customer_lookup  FROM _DBPRF_wc_customer_lookup LEFT JOIN _DBPRF_users ON _DBPRF_wc_customer_lookup.user_id = _DBPRF_users.ID WHERE _DBPRF_users.ID IS NULL"
			# del_customer_lookup_query = "DELETE FROM _DBPRF_wc_customer_lookup WHERE user_id > 0 "
		clear_table = self.get_connector_data(self.get_connector_url('query'), {
				'query': json.dumps({
					'type': 'query', 'query': del_customer_lookup_query
				})
	...
	def get_manufacturers_ext_export(self, manufacturers):
		...
		# 	manufacturers_ext_queries['woocommerce_termmeta'] = {
		# 		'type': 'select',
		# 		'query': "SELECT * FROM _DBPRF_wp_woocommerce_termmeta WHERE woocommerce_term_id IN " + self.list_to_in_condition(
		# 			category_ids) + " AND meta_key IN ('order', 'thumbnail_id', 'display_type')"
		# 	}
		...
		manufacturers_ext_rel_queries = {
			'post_meta': {
				...
				'query': "SELECT p.ID, p.post_title, pm.meta_value, p.guid FROM _DBPRF_posts AS p "
				         "LEFT JOIN _DBPRF_postmeta AS pm ON pm.post_id = p.ID AND pm.meta_key = '_attached_file' WHERE p.ID IN " + thumb_ids_query
			}
		}
		# add custom
	...
	def manufacturer_import(self, convert, manufacturer, manufacturers_ext):
		...
		if self.is_wpml() or self._notice['target']['support']['yoast_seo']:
			query_wpseo = {
				'type': 'select',
				'query': "SELECT * FROM `_DBPRF_options` WHERE `option_name` = 'seo_taxonomy_meta'"
			}
	...
	def get_categories_main_export(self):
		...
		# 		         # il.`element_type` = 'tax_product_cat' and il.`source_language_code` IS NULL and
		# 		         "WHERE  tt.taxonomy = 'product_cat' and tt.term_id > " + to_str(
		# 			id_src) + " ORDER BY tt.term_id ASC LIMIT " + to_str(limit),
		#
		# 	}
	...
	def category_import(self, convert, category, categories_ext):
		...
		if self._notice['target']['config'].get('product_cat'):
			taxonomy = self._notice['target']['config']['product_cat']
		category_taxonomy = {
			'term_id': term_id,
			'taxonomy': taxonomy,
		...
		return response_success(category_taxonomy_import)
	...
	def get_products_ext_export(self, products):
		...
		product_ext_rel_third_queries = {
			'image': {
				'type': 'select',
				'query': "SELECT p.ID, p.post_title, pm.meta_value, p.guid FROM _DBPRF_posts AS p "
				         "LEFT JOIN _DBPRF_postmeta AS pm ON pm.post_id = p.ID AND pm.meta_key = '_attached_file' "
	...
	def check_product_import(self, convert, product, products_ext):
		...
		if self.is_wpml() and not language_code:
			language_code = self._notice['target']['language_default']
		# lấy id của product để viết vào terms_relationship
		...
		if convert['attributes']:
			...
			for attribute in convert['attributes']:
				...
				'attribute_public': attribute['option_name'],
					...
					attribute_id = self.import_product_data_connector(attribute_query, True, convert['id'])
					if not attribute_id:
						return response_error('Attribute ' + to_str(attribute['option_id']) + ' import false.')
					self.insert_map(self.TYPE_ATTR, attribute['option_id'], attribute_id, attribute['option_code'],)
				attribute_map_id = self.get_map_field_by_src(self.TYPE_ATTR, attribute['option_id'], attribute['option_code'])
				# kiem tra gia tri attribute
				check_attribute_value_exist = True if self.select_map(self._migration_id, self.TYPE_ATTR_VALUE, None, None,attribute['option_set'], None, None, language_code) else False
				if check_attribute_value_exist == False:
					data_attribute_value = {
				'name': self.sanitize_title(attribute['option_set']),
				'slug': self.sanitize_title(attribute['option_set']),
				'term_group': 0 ,
			}
					...
					self.insert_map(self.TYPE_ATTR_VALUE, attribute_map_id, attribute_value_id, self.sanitize_title(attribute['option_set']),)
				else:
					attribute_value_id = self.get_map_field_by_src(self.TYPE_ATTR_VALUE, attribute_map_id, self.sanitize_title(attribute['option_set']))
				...
				# term_taxonomy_id_1 = self.get_map_field_by_src(self.TYPE_ATTR_OPTION, attribute_value_id, attr_taxonomy)
	...
	def product_import(self, convert, product, products_ext):
		...
		# parent_id = self.get_map_field_by_src(self.TYPE_PRODUCT, to_int(convert['parent']['parent_id']), None, language_code)
		# if not parent_id:
		# 	parent_id = 0
		data = {
		'post_author': 1,
		'post_date': convert['created_at'] if convert['created_at']  else '0000-00-00',
		'post_date_gmt': convert['created_at'] if convert['created_at'] else '0000-00-00',
		...
		'post_modified': convert['updated_at'] if convert['created_at'] and '0000-00-00' not in convert['updated_at'] else get_current_time(),
		'post_modified_gmt': convert['updated_at'] if convert['created_at'] and '0000-00-00' not in convert['updated_at'] else get_current_time(),
		...
		self.insert_map(self.TYPE_PRODUCT, convert['id'], product_id, convert['name'])
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		if convert['thumb_image']['path']:
			...
			if image_import_path:
				...
				if thumbnail_id:
					all_query.append(self.get_query_img_wpml(thumbnail_id, language_code))
		gallery_ids = list()
		img_id = False
		if convert['images']:
				images_process = self.process_image_before_import(convert['images'][0]['url'], convert['images'][0]['path'])
				images_import_path = self.uploadImageConnector(images_process, self.add_prefix_path(
					self.make_woocommerce_image_path(images_process['path']),
					self._notice['target']['config']['image_product'].rstrip('/')))
				if images_import_path:
					product_images = self.remove_prefix_path(images_import_path,
													self._notice['target']['config']['image_product'])
					images_details = self.get_sizes(images_process['url'])
					img_id = self.wp_image(product_images, images_details, convert['images'][0].get('label',''))
					if img_id:
						gallery_ids.append(img_id)
						if self.is_wpml():
							all_query.append(self.get_query_img_wpml(img_id, language_code))
		stock_status = 'instock'
		product_meta = {
			...
			'_product_image_gallery' : img_id if img_id else '',
			'_manage_stock': 'yes' if convert['manage_stock'] or convert['manage_stock'] == True else 'no',
		}
	...
	def get_customers_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM " + prefix + "users u LEFT JOIN " + prefix + "usermeta um ON u.ID = um.user_id WHERE (um.meta_key = 'capabilities' AND um.meta_value LIKE '%customer%' OR um.meta_value LIKE '%subscriber%') AND ID > " + to_str(id_src) + " ORDER BY ID ASC LIMIT " + to_str(limit)
		}
	...
	def customer_import(self, convert, customer, customers_ext):
		# slug = self.sanitize_title(convert['id'])
		...
		admin_user = {
			...
			'user_nicename': convert['first_name'] + ' ' + convert['last_name'],
		...
		admin_user_query = self.create_insert_query_connector('users', admin_user)
		user_id = self.import_data_connector(admin_user_query, 'customer')

		if not user_id:
			return response_warning('Customer ' + to_str(convert['id']) + ' import false.')
		meta_key_1 = prefix + 'capabilities'
		customer_usermeta_1 = {
			'user_id': user_id,
			'meta_key': meta_key_1,
			'meta_value': 'a:1:{s:8:\"customer\";b:1;}'
		}
		customer_usermeta_1_query = self.create_insert_query_connector('usermeta', customer_usermeta_1)
		customer_usermeta_1_import = self.import_manufacturer_data_connector(customer_usermeta_1_query, True, convert['id'])
		if not customer_usermeta_1_import:
			return response_warning('Customer ' + to_str(convert['id']) + ' import false.')
		meta_key_2 = 'nickname'
		customer_usermeta_2 = {
			'user_id': user_id,
			'meta_key': meta_key_2,
			'meta_value': convert['first_name'] + ' ' + convert['last_name']
		}
		customer_usermeta_2_query = self.create_insert_query_connector('usermeta', customer_usermeta_2)
		customer_usermeta_2_import = self.import_customer_data_connector(customer_usermeta_2_query, True, convert['id'])
		if not customer_usermeta_2_import:
			return response_warning('Customer ' + to_str(convert['id']) + ' import false.')
		meta_key_3 = 'first_name'
		customer_usermeta_3 = {
			'user_id': user_id,
			'meta_key': meta_key_3,
			'meta_value': convert['first_name']
		}
		customer_usermeta_3_query = self.create_insert_query_connector('usermeta', customer_usermeta_3)
		customer_usermeta_3_import = self.import_customer_data_connector(customer_usermeta_3_query, True, convert['id'])
		if not customer_usermeta_3_import:
			return response_warning('Customer ' + to_str(convert['id']) + ' import false.')
		meta_key_4 = 'last_name'
		customer_usermeta_4 = {
			'user_id': user_id,
			'meta_key': meta_key_4,
			'meta_value': convert['last_name']
		}
		customer_usermeta_4_query = self.create_insert_query_connector('usermeta', customer_usermeta_4)
		customer_usermeta_4_import = self.import_customer_data_connector(customer_usermeta_4_query, True, convert['id'])
		if not customer_usermeta_4_import:
			return response_warning('Customer ' + to_str(convert['id']) + ' import false.')
		self.insert_map(self.TYPE_CUSTOMER, convert['id'], user_id, convert['first_name'])
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		...
		if convert['address']:
			...
			if not billing_data:
				billing_data = convert['address'][0]
			if not shipping_data:
				shipping_data = convert['address'][1] if 1 in convert['address'] else billing_data
			user_meta = {
				'billing_country': billing_data['country']['country_code'],
				...
				'billing_state': billing_data['state']['name'],
				'billing_postcode': billing_data['postcode'],
				'billing_email': convert['email'],
				'billing_phone': billing_data['telephone'],
				'billing_fax': billing_data['fax'],
				'shipping_country': shipping_data['country']['country_code'],
				...
				'shipping_state': shipping_data['state']['name'],
				'shipping_postcode': shipping_data['postcode'],
				'shipping_email': convert['email'],
				'shipping_phone': shipping_data['telephone'],
				'shipping_fax': shipping_data['fax'],
			}
		...
		admin_lookup = {
			'user_id' : customer_id,
			'username' : convert['first_name'] + convert['last_name'],
		...
		admin_lookup_query = self.create_insert_query_connector('wc_customer_lookup', admin_lookup)
		admin_lookup_import = self.import_customer_data_connector(admin_lookup_query, True, convert['id'])
		if not admin_lookup_import:
			return response_warning('Customer ' + to_str(convert['id']) + ' import false.')
		return response_success()
